...
public class ViewBehavior extends CoordinatorLayout.Behavior<HeaderView> {
    ...
    private void shouldInitProperties(HeaderView child, View dependency) {
        ...
        if (mStartMarginLeft == 0)
            mStartMarginLeft = mContext.getResources().getDimensionPixelOffset(R.dimen._14sdp);

        if (mEndMargintLeft == 0)
            mEndMargintLeft = mContext.getResources().getDimensionPixelOffset(R.dimen._14sdp);

        if (mStartMarginBottom == 0)
            mStartMarginBottom = mContext.getResources().getDimensionPixelOffset(R.dimen._14sdp);

        if (mMarginRight == 0)
            mMarginRight = mContext.getResources().getDimensionPixelOffset(R.dimen._14sdp);
