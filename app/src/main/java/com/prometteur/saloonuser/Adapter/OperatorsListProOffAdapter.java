...
public class OperatorsListProOffAdapter extends RecyclerView.Adapter<OperatorsListProOffAdapter.ViewHolder> {

    Activity nActivity;
    private final OnItemClickListener listener;
    int selectedPosition = -1;
    List<PromoOfferBean.Operator> operators;

    public OperatorsListProOffAdapter(Activity nActivity, List<PromoOfferBean.Operator> operators, OperatorsListProOffAdapter.OnItemClickListener onItemClickListener) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(nActivity).inflate(R.layout.recycle_operators_list,
                parent, false);
        return new OperatorsListProOffAdapter.ViewHolder(view);
    }
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
       ...
       // holder.bind(position, listener, holder.tvOperatorName.getText().toString(), holder.conlayMainOperatorList);
        ...
        Glide.with(nActivity).load(operators.get(position).getUserImg()).placeholder(R.drawable.img_profile).error(R.drawable.img_profile).into(holder.civ_OperatorImage);
